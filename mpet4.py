...
URL_PREFIX = 'https://results.eci.gov.in/ResultAcGenDec2022/hi/ConstituencywiseS06'
...
def is_valid_ac_identifier(identifiers):
    # Check if at least one identifier is either a valid AC number or a valid AC name (including partial matches)
    return any(is_valid_ac_number(identifier) or any(fuzz.partial_ratio(identifier.lower(), ac.lower()) >= 80 for ac in ac_mapping) for identifier in identifiers)

@bot.message_handler(func=lambda message: is_valid_ac_identifier(message.text.split(',')))
        ...
        for identifier in identifiers:
            ...
            if ac_number is not None:
                ...
                if response.status_code == 200:
                    ...
                    result_text = f"\n\n<b>{ac_name} (AC {ac_number}):</b>\n\n"
                    result_text += "\n\n".join([
                        f"{i + 1}.{entry['Candidate']}\n<b>{entry['Party']}</b>\n<b>{entry['Total Votes']}</b>"
                ...
                if matches[1] >= 80:
                    ...
                    if response.status_code == 200:
                        ...
                        result_text = f"\n\n<b>{suggested_ac_number}:</b><b>{ac_name}</b>\n\n"
                        result_text += "\n\n".join([
                            f"{i + 1}. {entry['Candidate']}\n<b>{entry['Party']}</b>\n<b>{entry['Total Votes']}</b>"
                            for i, entry in enumerate(sorted_data)
                        ])
